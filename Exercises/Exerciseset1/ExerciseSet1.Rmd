...
**Your Answers here:**
<br>#Q1 r source("dataSet1.R")
<br>Especially, in the below line of "the "dataSet1.R": 
tmpdata<-as.matrix(try(getSymbols(Symbols=ticker_to_get,from = startDate,auto.assign=FALSE))). 

<br>#Q2 
<br>the cumulative return  r round(100*sum(StockReturns[,2]),1)
<br>the average r round(100*mean(StockReturns[,2]),3)
<br>the standard deviation r round(100*sd(StockReturns[,2]),3)
<hr>
<hr>
...
**Your Answers here:**
<br>#Q1 the number of rows: nrow(transposedData) 
<br>the number of columns: ncol(transposedData) 
<br>#Q2 portfolio <- apply(transposedData,2,mean)
<br>names(portfolio) <- rownames(StockReturns)
<br>pnl_plot(portfolio)
...
**Your Answers here:**
<br>Q1 Modyfy the line in dataSet1.R file: startDate = "2001-01-01"
<br>Q2 Suppose I want to add Google (ticker: GOOG) and Facebook (ticker: FB) in my analysis. In "dataSet1.R" file, modify the line:
<br>mytickers = c("SPY", "AAPL", "GOOG", "FB")
<br>And then run 2 following commands:
<br>Newportfolio <- matrix(c(StockReturns[,"AAPL"],StockReturns[,"GOOG"],StockReturns[,"FB"]),  length(StockReturns[,"AAPL"]), 3)
<br>apply(Newportfolio,1,mean)
...
**Your Answers here:**
<br>Q1 sum(myData != StockReturns[1:20,]), the command would return 60, which is the product of the number of columns and raws.
<br>The command is the results of the logical equation whether my Data's element is identical to the matrix of StockReturns. In this case, the read function used the separator "," rather than ";", and all equation returns TRUE (i.e. all elements are not equal).
<br>Even if the csv file were correctly read by using the separator ";", the command would return 20, because the column of dates in CSV would be regarded as text, not numerical data. 
